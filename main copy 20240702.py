...
from panorama.panorama import (stitch, crop, add_black_margin, complement_sky)
...
class AMR(ADSClient):
    ...
    def do_routine_job(self, client):
        ...
        # initalize amr state
        while not self.initialize_state:
            ...
            try:
                ...
                if CameraWorkStatus == 0 and ToCameraWorkCommand == 0 and FromCameraWorkCommand == 0: 
                    # initialize completed
                    self.initialize_state = True
                    self.logger.info("------------initialize ok!------------")
                    break
            time.sleep(1)
            continue
        ...
        try:
            ...
            self.logger.error("retrieve ads data failed!")
            time.sleep(1)
            success = False
        else:
                    ...
                    if "status" in response and "message" in response:
                        if response.get("status"):
                            ...
                            try:
                                ...
                                client.write_by_name("GVL.FromCameraWorkCommand", 7)
                            except:
                                success = False
                                return success
                            else:
                    ...
                    if "status" in response and "message" in response:
                        if response.get("status"):
                            ...
                            try:
                                ...
                                client.write_by_name("GVL.FromCameraWorkCommand", 8)
                            except:
                                success = False
                                return success
                            else:
                ...
                if ret and isinstance(response, Iterable):
                    if "status" in response and "message" in response:
                        if response.get("status"):
                            ...
                            try:
                                client.write_by_name("GVL.CameraWorkStatus", 1)  # CameraWorkStatus 1
                                client.write_by_name("GVL.FromCameraWorkCommand", 9)
                            except:
                                success = False
                                return success
                            else:
                ...
                if ret and isinstance(response, Iterable):
                    if "status" in response and "message" in response:
                        if response.get("status"):
                            ...
                            try:
                                client.write_by_name("GVL.CameraWorkStatus", 1)  # CameraWorkStatus????1,?????
                                client.write_by_name("GVL.FromCameraWorkCommand", 10)
                            except:
                                success = False
                                return success
                            else:
        ...
        # elif ToCameraWorkCommand >= 1100 and ToCameraWorkCommand < 1200: # video task
            ...
            if CameraWorkStatus == 0:
            ...
            elif CameraWorkStatus == 2:

                ################ abnormal complete the task ################
                self.logger.debug("abnormal complete the task")
                try:
                    client.write_by_name("GVL.CameraWorkStatus", 0)
                    client.write_by_name("GVL.FromCameraWorkCommand", 0)
                except:
                ...
                self.logger.error("CameraWorkStatus error!") 
    ...
    def move_amr(self, cmd):
        ...
        try:
            if cmd == "stop":
                # self.__client.write_by_name(xxxxx, 0)
                self.logger.debug("stop amr!")
            elif cmd == "forward":
                # self.__client.write_by_name(xxxxx, 1)
                self.logger.debug("move forward!")
            elif cmd == "left-turn":
                # self.__client.write_by_name(xxxxx, 2)
        ...
        for task in os.listdir("save_imgs" + os.sep + pos):
            ...
            if ret1:
                ...
                try:
                    ...
                    camera.main_logger.error(f"read task folder failed, pos:{pos}, task:{task}.")
                else:
                    ...
                    for line in lines:
                        if "task_type" in line:
                            ...
                            camera.main_logger.error(f"can not get task type!")
                            break
                        if "amr_pos_x" in line:
                            amr_pos_x = line.replace("amr_pos_x:", "")
                        else:
                            camera.main_logger.error(f"can not get amr_pos_x!")
                            break
                        if "amr_pos_y" in line:
                            amr_pos_y = line.replace("amr_pos_y:", "")
                        else:
                            camera.main_logger.error(f"can not get amr_pos_y!")
                            break
                        if "amr_pos_z" in line:
                            amr_pos_z = line.replace("amr_pos_z:", "")
                        else:
                            camera.main_logger.error(f"can not get amr_pos_z!")
                            break
                        if "amr_pos_theta" in line:
                            amr_pos_theta = line.replace("amr_pos_theta:", "")
                        else:
                            camera.main_logger.error(f"can not get amr_pos_theta!")
                        if "amr_tag_id" in line:
                            amr_tag_id = line.replace("amr_tag_id:", "")
                        else:
                            camera.main_logger.error(f"can not get amr_tag_id!")
                            break
                        if "ftp_url" in line:
                            ftp_url = line.replace("ftp_url:", "") 
                        else:
                            camera.main_logger.error(f"can not get ftp_url!")                      
                        if "stitch_state:" in line:
                            stitch_state = line.replace("stitch_state:", "")
                        else:
                            camera.main_logger.error(f"can not get stitch_state!")
                            break 
                        if "requestor" in line:
                            requestor = line.replace("requestor:", "")
                        else:
                            camera.main_logger.error(f"can not get requestor!")
                            break
                        if "task_time" in line:
                            task_time = line.replace("task_time:", "")
                        else:
                            camera.main_logger.error(f"can not get task_time!")
                            break
