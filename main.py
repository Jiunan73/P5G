...
class AMR(ADSClient):
    ...
    def __init__(self, ams_net_id: str = None):
        ...
        # 替身任務切換
        self.mode_change = ""  # "", manual, auto
        self.change_mode_executor = ThreadPoolExecutor(max_workers=5)
        self.ManualSemiControlDisableStatus = None  # False: 可切換替身; True:不可切換替身
        self.bWeb_bBeckhoff_IPC_HartBitStatus = None  # False: ?????; True:??????
        self.AutoManualStatus = None  # False:替身模式; True:自動模式
        # 將AutoManualSwitch切換至True，並等待AutoManualStatus = False，完成切換
        self.AutoManualSwitch = None
        # 替身網頁控制心跳
        self.ManualSemiControlHartBit = None
        self.web_control_counter = 0
        self.web_control_thread = Thread(target=self.web_control_heartbeat)
        self.web_control_thread.start()
        # 替身任務 flag, 前進、後退、左轉、右轉控制
        self.ManualSemiControlEnable = None  # 0或1，執行替身任務時為1，放開為0
        self.ManualSemiControlForwardButton = None  # 0或1，前進時為1，放開為0
        self.ManualSemiControlBackwardButton = None  # 0或1，前進時為1，放開為0
        self.ManualSemiControlLeftButton = None  # 0或1，左轉時為1，放開為0
        self.ManualSemiControlRightButton = None  # 0或1，右轉為1，放開為0
        self.web_control_stop_thread = Thread()  # 停止失敗時，執行stop至成功為止
        # 搖桿模式
        self.ManualJoyControlEnable = None  # False：，True：可否切換替身模式
        self.ManualJoyDirection = None  # 替身模式方向(DINT)
        self.ManualJoyStrength = None  # 替身模式強度(INT)

        # IUMOBO 障礙偵測照片上傳
        self.LastRealSenseObstacleSignal = None  # 上一次訊號
        self.RealSenseObstacleSignal = None  # 此次訊號
    ...
    def do_routine_job(self, client):
        ...
        if isinstance(pan, float) and isinstance(tilt, float) and isinstance(zoom, float):
            self.camera.crr_pan, self.camera.crr_tilt, self.camera.crr_zoom = calculate_ptz_angle([
                                                                                                  pan, tilt, zoom])

        # amr heart beat
        try:
            client.write_by_name(
                "GVL.ExternalDevice1.bFromExternalDeviceHartBit", self.heart_bit)
        except:
            self.logger.error("write heart beat failed!")
            success = False
            return success
        finally:
            if self.heart_bit == 1:
        ...
        try:
            ...
            CameraWorkStatus = client.read_by_name(
                "GVL.CameraWorkStatus")  # camera status
            ToCameraWorkCommand = client.read_by_name(
                "GVL.ToCameraWorkCommand")
            FromCameraWorkCommand = client.read_by_name(
                "GVL.FromCameraWorkCommand")
        except Exception as e:
            self.logger.error(f"read amr data failed, error:{e}")
            success = False
            return success
        else:
            ...
            self.amr_pos_theta = pos_theta // 1000  # 取整數

        # 替身任務
        try:
            ManualSemiControlDisableStatus = client.read_by_name(
                "GVL.bWeb_ManualSemiControlDisableStatus")
            ...
            bWeb_bBeckhoff_IPC_HartBitStatus = client.read_by_name("GVL.bWeb_bBeckhoff_IPC_HartBitStatus")
            ManualJoyControlEnable = client.read_by_name(
                "GVL.bWeb_ManualJoyControlEnable")  # 是否可切換替身模式
            ManualJoyDirection = client.read_by_name(
                "GVL.bWeb_ManualJoyDirection")  # 替身模式方向
            ManualJoyStrength = client.read_by_name(
                "GVL.bWeb_ManualJoyStrength")  # 替身模式強度
        except Exception as e:
            self.logger.error(f"read manual function failed, error:{e}")
            # success = False
            # return success
        else:
            # 替身任務狀態
            self.bWeb_bBeckhoff_IPC_HartBitStatus = bWeb_bBeckhoff_IPC_HartBitStatus
            ...
            # 搖桿控制
            self.ManualJoyControlEnable = ManualJoyControlEnable  # 可否切換替身模式
            self.ManualJoyDirection = ManualJoyDirection  # 替身模式方向
            self.ManualJoyStrength = ManualJoyStrength  # 替身模式強度

        # IUMOBO 障礙物偵測
        try:
            RealSenseObstacleSignal = client.read_by_name(
                "GVL.bWeb_RealSenseObstacleSignal")
        except Exception as e:
            self.logger.error(
                f"read iumobo obstacle signal failed!, error:{e}")
            # success = False
            # return success
        else:
            # IUMOBO 障礙物偵測
        ...
        # report status
        self.logger.info(f"heart beat:{self.heart_bit}")
        self.logger.info(
            f"FromCameraWorkCommand:{FromCameraWorkCommand}, ToCameraWorkCommand:{ToCameraWorkCommand}, CameraWorkStatus:{CameraWorkStatus}")
        self.logger.info(
            f"pan:{self.camera.crr_pan}, tilt:{self.camera.crr_tilt}, zoom:{self.camera.crr_zoom}")
        self.logger.info(
            f"pos_x:{self.amr_pos_x}, pos_y:{self.amr_pos_y}, pos_theta:{self.amr_pos_theta}, tag_id:{tag_id}, pos_z:{pos_z}")
        self.logger.info(
            f"Max Temperature:{camera.ir_cam.max_temperature_float}, max_coords:{camera.ir_cam.max_coords}")
        # report task status
        self.logger.info(
            f"target_task:{self.camera.target_task.is_running}, ir_task:{self.camera.ir_task.is_running}, designated_task:{self.camera.designated_task.is_running}, video_task:{self.camera.video_task.is_running}, initial_task:{self.camera.initial_task.is_running}")

        self.logger.info(
            f"ManualSemiControlDisableStatus:{self.ManualSemiControlDisableStatus}, AutoManualStatus:{self.AutoManualStatus}, AutoManualSwitch:{self.AutoManualSwitch}")

        # report realsense obstacle status
        self.logger.info(
            f"RealSenseObstacleSignal:{self.RealSenseObstacleSignal}, LastRealSenseObstacleSignal:{self.LastRealSenseObstacleSignal}")

        # # 切換替身任務完成
        # if self.AutoManualStatus == False and self.AutoManualSwitch == True and self.mode_change == "manual":
        #     # AutoManualSwitch切回False，切換替身任務完成
        #     self.logger.debug("AutoManualSwitch切回False，切換替身任務完成")
        #     try:
        #         client.write_by_name("GVL.bWeb_AutoManualSwitch", False)
        #     except Exception as e:
        #         self.logger("change to manual mode failed")
        #         return success
        #     else:
        #         self.mode_change = ""
        #         self.logger("change to manual mode successfully!")

        # elif self.AutoManualStatus == True and self.AutoManualSwitch == True and self.mode_change == "auto":
        #     # AutoManualSwitch切回False，切換自動模式完成
        #     self.logger.debug("AutoManualSwitch切回False，切換自動模式完成")
        #     try:
        #         client.write_by_name("GVL.bWeb_AutoManualSwitch", False)
        #     except Exception as e:
        #         self.logger("change to auto mode failed")
        #         return success
        #     else:
        #         self.mode_change = ""
        #         self.logger("change to auto mode successfully!")

        # IUMOBO 障礙物偵測工作
        if self.RealSenseObstacleSignal == True and self.LastRealSenseObstacleSignal == False:
            # download and upload obstacle imgs
            self.obstacle_thread = Thread(
                target=self.download_and_upload_obstacle_imgs)
        ...
        if ToCameraWorkCommand == 7:  # designated task
            ...
            if CameraWorkStatus == 0:
                ...
                if FromCameraWorkCommand == 0:
                    ...
                    self.logger.debug(
                        "start designated task, write FromCameraWorkCommand = 7, and write CameraWorkStatus = 1")
                    ...
                    self.logger.debug(
                        f"request result----ret:{ret}, response:{response}")
                    if ret and isinstance(response, Iterable):
                        if "status" in response and "message" in response:
                            if response.get("status"):
                                ...
                                try:
                                    client.write_by_name(
                                        "GVL.CameraWorkStatus", 1)
                                    client.write_by_name(
                                        "GVL.FromCameraWorkCommand", ToCameraWorkCommand)
                                except:
                                    success = False
                                    return success
                                else:
                                    self.logger.debug(
                                        "start designated task successfully!")
                            else:

                                # status failed
                                self.logger.error(
                                    "status failed, write CameraWorkStatus = 2, FromCameraCommand = 7")
                            ...
                            # type failed
                            self.logger.error(
                                "type failed, write CameraWorkStatus = 2, FromCameraCommand = 7")
                        ...
                        # request failed
                        self.logger.error(
                            "request failed, write CameraWorkStatus = 2, FromCameraCommand = 7")
                ...
                if not self.camera.designated_task.is_running:
                    # task completed!
                    self.logger.debug(
                        "designated task completed, write CameraWorkStatus = 0")
                    # PLC D???
                    try:
            ...
            if CameraWorkStatus == 0:
                ...
                if FromCameraWorkCommand == 0:
                    ...
                    ################ start ir task ################
                    self.logger.debug(
                        "start ir task, write FromCameraWorkCommand = 8, and write CameraWorkStatus = 1")
                    ...
                    if ret and isinstance(response, Iterable):
                        if "status" in response and "message" in response:
                            if response.get("status"):
                                ...
                                try:
                                    ...
                                    self.logger.debug(
                                        "start ir task successfully!")
                            else:

                                # status failed
                                self.logger.error(
                                    "status failed, write CameraWorkStatus = 2, FromCameraCommand = 8")
                            ...
                            # type failed
                            self.logger.error(
                                "type failed, write CameraWorkStatus = 2, FromCameraCommand = 8")
                        ...
                        # request failed
                        self.logger.error(
                            "request failed, write CameraWorkStatus = 2, FromCameraCommand = 8")
                ...
                if not self.camera.ir_task.is_running:
                    # task completed!
                    self.logger.debug(
                        "ir task completed, write CameraWorkStatus = 0")
                    # PLC D???
                    try:
                        client.write_by_name("GVL.CameraWorkStatus", 0)
                    except:
            ...
            if CameraWorkStatus == 0:
                ...
                if FromCameraWorkCommand == 0:
                    ...
                    ################ start target task ################
                    self.logger.debug(
                        "start target task, write FromCameraWorkCommand = 9, and write CameraWorkStatus = 1")
                    ...
                    if ret and isinstance(response, Iterable):
                        if "status" in response and "message" in response:
                            if response.get("status"):
                                ...
                                try:
                                    ...
                                    self.logger.debug(
                                        "start target task successfully!")
                            else:
                                # status failed
                                self.logger.error(
                                    "status failed, write CameraWorkStatus = 2, FromCameraCommand = 9")
                            ...
                            # type failed
                            self.logger.error(
                                "type failed, write CameraWorkStatus = 2, FromCameraCommand = 9")
                        ...
                        # request failed
                        self.logger.error(
                            "request failed, write CameraWorkStatus = 2, FromCameraCommand = 9")
                ...
                if not self.camera.target_task.is_running:
                    # task completed!
                    self.logger.debug(
                        "target task completed, write CameraWorkStatus = 0")
                    try:
                        # CameraWorkStatus 0, complted
                        client.write_by_name("GVL.CameraWorkStatus", 0)
            ...
            if CameraWorkStatus == 0:
                ...
                if FromCameraWorkCommand == 0:
                    ...
                    ################ start panorama task ################
                    self.logger.debug(
                        "start panorama task, write FromCameraWorkCommand = 10, and write CameraWorkStatus = 1")
                    ...
                    if ret and isinstance(response, Iterable):
                        if "status" in response and "message" in response:
                            if response.get("status"):
                                ...
                                try:
                                    ...
                                    self.logger.debug(
                                        "start panorama task successfully!")
                            else:

                                # status failed
                                self.logger.error(
                                    "status failed, write CameraWorkStatus = 2, FromCameraCommand = 10")
                            ...
                            # type failed
                            self.logger.error(
                                "type failed, write CameraWorkStatus = 2, FromCameraCommand = 10")
                        ...
                        # request failed
                        self.logger.error(
                            "request failed, write CameraWorkStatus = 2, FromCameraCommand = 10")
                ...
                if not self.camera.panorama_task.is_running:
                    # task completed!
                    self.logger.debug(
                        "panorama task completed, write CameraWorkStatus = 0")
                    # PLC D
                    try:
                        client.write_by_name("GVL.CameraWorkStatus", 0)
                    except:
            ...
            if CameraWorkStatus == 0:
                ...
                if FromCameraWorkCommand == 0:
                    ...
                    ################ start video task ################
                    self.logger.debug(
                        f"start video task, write FromCameraWorkCommand = {ToCameraWorkCommand}, and write CameraWorkStatus = 1")
                    ...
                    if ret and isinstance(response, Iterable):
                        if "status" in response and "message" in response:
                            if response.get("status"):
                                ...
                                try:
                                    ...
                                    self.logger.debug(
                                        "start video task successfully!")
                            else:
                                # status failed
                                self.logger.error(
                                    f"status failed, write CameraWorkStatus = 2, FromCameraCommand = {ToCameraWorkCommand}")
                            ...
                            # type failed
                            self.logger.error(
                                f"type failed, write CameraWorkStatus = 2, FromCameraCommand = {ToCameraWorkCommand}")
                        ...
                        # request failed
                        self.logger.error(
                            f"request failed, write CameraWorkStatus = 2, FromCameraCommand = {ToCameraWorkCommand}")
                ...
                if not self.camera.video_task.is_running:
                    # task completed!
                    self.logger.debug(
                        "video task completed, write CameraWorkStatus = 0")
            ...
            if CameraWorkStatus == 0:
                ...
                if FromCameraWorkCommand == 0:
                    ...
                    ################ start initial task ################
                    self.logger.debug(
                        "start initial task, write FromCameraWorkCommand = 12, and write CameraWorkStatus = 1")
                        ...
                        if "status" in response and "message" in response:
                            if response.get("status"):
                                ...
                                try:
                                    ...
                                    self.logger.debug(
                                        "start initial task successfully!")
                            else:
                ...
                if not self.camera.initial_task.is_running:
                    # task completed!
                    self.logger.debug(
                        "initial task completed, write CameraWorkStatus = 0")
                    try:
                        client.write_by_name("GVL.CameraWorkStatus", 0)
                    except:
                        success = False
            ...
            if CameraWorkStatus == 0:
                ...
                if FromCameraWorkCommand != 0:  # 7, 8, 9, 10, 11, 12
                    ...
                    ################ complete the task ################
                    self.logger.debug(
                        "complete the task, write FromCameraWorkCommand = 0")
                    try:
                        client.write_by_name("GVL.FromCameraWorkCommand", 0)
                    except:
                        success = False
        ...
        # record the last signal
        self.LastRealSenseObstacleSignal = self.RealSenseObstacleSignal
    ...
    def add_camera(self, camera):
        """add camera"""
        self.camera = camera
    def change_mode_joy(self):
        #Jiunan
        self.mode_change = ""
        self.client.write_by_name("GVL.bWeb_ManualSemiControlEnable", True)
        self.client.write_by_name("GVL.bWeb_AutoManualSwitch", True)
        time.sleep(0.5)
        self.client.write_by_name("GVL.bWeb_ManualSemiControlEnable", False)
        self.client.write_by_name("GVL.bWeb_AutoManualSwitch", False)     
    def change_to_manual_mode(self):
        """切換替身模式"""
        success = False
        self.logger.debug("change to manual mode")

        # False: 可切換替身; True:不可切換替身
        if self.ManualSemiControlDisableStatus == True:
            self.logger.debug(
                "ManualSemiControlDisableStatus = False, can not change to manual mode")
            return success

        # False：替身任務狀態，無需切換
        if self.AutoManualStatus == False:
            self.logger.debug("AutoManualStatus = False, not need to change")
            success = True
            return success

        # 切換替身任務，並等待AutoManualStatus為False
        try:
            self.mode_change = "manual"
            self.client.write_by_name("GVL.bWeb_ManualSemiControlEnable", True)
            self.client.write_by_name("GVL.bWeb_AutoManualSwitch", True)
        except Exception as e:
            self.logger.error(f"change manual mode failed, error:{e}")
            self.mode_change = ""
        else:
            i = 0
            while True:
                # 等待AutoManualStatus為False
                if self.AutoManualStatus == False:
                    success = True
                    break
                if i > 10:
                ...
                i += 1
                time.sleep(1)
        # 不管切成功，或失敗，訊號都要切回False
        try:
            self.mode_change = ""
            self.client.write_by_name(
                "GVL.bWeb_ManualSemiControlEnable", False)
            self.client.write_by_name("GVL.bWeb_AutoManualSwitch", False)
        except Exception as e:
            self.logger.error(f"change auto mode failed, error:{e}")
            # message = e
            self.mode_change = ""
        self.logger.error(f"change manual mode:{success}")
        return success

    def change_to_auto_mode(self):
        """切換回自動模式"""
        success = False
        self.logger.debug("change to auto mode")

        # False: 可切換替身; True:不可切換替身
        if self.ManualSemiControlDisableStatus == True:
            self.logger.debug(
                "ManualSemiControlDisableStatus = False, can not change to manual mode")
            message = "ManualSemiControlDisableStatus = False"
            return success

        # True：自動狀態，無需切換
        if self.AutoManualStatus == True:
            self.logger.debug(
                "AutoManualStatus = True, no need to change mode")
            success = True
            # message = "AutoManualStatus = True, no need to change mode"
            return success

        # 切換自動狀態，並等待self.AutoManualStatus回True
        try:
            self.mode_change = "auto"
            self.client.write_by_name("GVL.bWeb_ManualSemiControlEnable", True)
            self.client.write_by_name("GVL.bWeb_AutoManualSwitch", True)
        except Exception as e:
            self.logger.error(f"change auto mode failed, error:{e}")
        ...
        try:
            ...
            # message = e
            self.mode_change = ""
        self.logger.error(f"change auto mode:{success}")
        return success

    def move_amr(self, cmd, angle=0, strength=0):
        """move amr"""
        success = False
        ...
        try:
            if cmd == "stop":
                ...
                try:
                    self.client.write_by_name(
                        "GVL.bWeb_ManualSemiControlEnable", 0)  # flag
                    self.client.write_by_name(
                        "GVL.bWeb_ManualSemiControlForwardButton", 0)  # forward stop
                    self.client.write_by_name(
                        "GVL.bWeb_ManualSemiControlBackwardButton", 0)  # backward stop
                    self.client.write_by_name(
                        "GVL.bWeb_ManualSemiControlLeftButton", 0)  # left turn stop
                    self.client.write_by_name(
                        "GVL.bWeb_ManualSemiControlRightButton", 0)  # right turn stop
                except Exception as e:
                    self.logger.error("manual stop control failed!")
                    # send stop until successful
                    if not self.web_control_stop_thread.is_alive():
                        self.web_control_stop_thread = Thread(
                            target=self.stop_amr_until_successful)
                        self.web_control_stop_thread.start()
                    success = False
                else:
                    # 更新flag, 前進、後退、左轉、右轉狀態值為0
                    self.ManualSemiControlEnable = 0
                    self.ManualSemiControlForwardButton = 0
                    self.ManualSemiControlBackwardButton = 0
                    self.ManualSemiControlLeftButton = 0
                    self.ManualSemiControlRightButton = 0
                    success = True
                ...
                try:
                    self.client.write_by_name(
                        "GVL.bWeb_ManualSemiControlEnable", 1)  # flag
                    self.client.write_by_name(
                        "GVL.bWeb_ManualSemiControlForwardButton", 1)  # forward
                except Exception as e:
                    self.logger.error("manual forward control failed!")
                    success = False
                else:
                    # 更新flag 1, 前進1
                    self.ManualSemiControlEnable = 1
                    self.ManualSemiControlForwardButton = 1
                    success = True
                ...
                try:
                    self.client.write_by_name(
                        "GVL.bWeb_ManualSemiControlEnable", 1)  # flag
                    self.client.write_by_name(
                        "GVL.bWeb_ManualSemiControlBackwardButton", 1)  # backward
                except Exception as e:
                    self.logger.error("manual forward control failed!")
                    success = False
                else:
                    # 更新flag 1, 後退1
                    self.ManualSemiControlEnable = 1
                    self.ManualSemiControlBackwardButton = 1
                    success = True
                ...
                try:
                    self.client.write_by_name(
                        "GVL.bWeb_ManualSemiControlEnable", 1)  # flag
                    self.client.write_by_name(
                        "GVL.bWeb_ManualSemiControlLeftButton", 1)  # left turn
                except Exception as e:
                    ...
                    # 更新flag 1, 左轉1
                    self.ManualSemiControlEnable = 1
                    self.ManualSemiControlLeftButton = 1
                    success = True
                ...
                try:
                    self.client.write_by_name(
                        "GVL.bWeb_ManualSemiControlEnable", 1)  # flag
                    self.client.write_by_name(
                        "GVL.bWeb_ManualSemiControlRightButton", 1)  # right turn
                except Exception as e:
                    ...
                    # 更新flag 1, 右轉1
                    self.ManualSemiControlEnable = 1
                    self.ManualSemiControlRightButton = 1
                    success = True

            elif cmd == "joystick":
                self.logger.debug(
                    f"joystick control, angle:{angle}, strength:{strength}")
                try:
                    ...
                    self.client.write_by_name(
                        "GVL.bWeb_ManualJoyControlEnable", control_enabled)  # 是否可切換替身模式
                    self.client.write_by_name(
                        "GVL.bWeb_ManualJoyStrength", strength)  # 
                    self.client.write_by_name(
                        "GVL.bWeb_ManualJoyDirection", angle)  # 替身模式方向
                    self.client.write_by_name(
                        "GVL.bWeb_ManualJoyStrength", strength)  # 替身模式強度
                    ...
                    # 更新
                    self.ManualJoyControlEnable = control_enabled
                    self.ManualJoyDirection = angle
                    self.ManualJoyStrength = strength
                    success = True

            elif cmd == "manual-mode":
                self.logger.debug("manual mode!")
                future = self.change_mode_executor.submit(
                    self.change_to_manual_mode)
                # future = self.change_mode_executor.submit(
                #     self.change_to_auto_mode)
                success = future.result()
                # success = self.change_to_manual_mode()  # 執行切換替身模式

            elif cmd == "auto-mode":
                self.logger.debug("auto mode!")
                future = self.change_mode_executor.submit(
                    self.change_to_auto_mode)
                # future = self.change_mode_executor.submit(
                #     self.change_to_manual_mode)
                success = future.result()
                # success = self.change_to_auto_mode()  # 執行切換自動模式
            else:
        ...
        self.logger.debug(f"move status:{success}")
        return success
    ...
    def stop_amr_until_successful(self):
        ...
        while True:
            try:
                ...
                self.logger.error(f"manual stop control failed, error:{e}!")
            else:
                # 更新flag, 前進、後退、左轉、右轉狀態值為0
                self.ManualSemiControlEnable = 0
                self.ManualSemiControlForwardButton = 0
        ...
        while True:
            try:
                if self.client is not None:
                    self.client.write_by_name(
                        "GVL.bWeb_ManualSemiControlHartBit", self.web_control_counter)
            except Exception as e:
                self.logger.error(f"write web hearteat failed, error:{e}")
            else:
                # 更新heart beat
                self.ManualSemiControlHartBit = self.web_control_counter
            finally:
                if self.web_control_counter == 0:
                    self.web_control_counter = 1
                else:
                    self.web_control_counter = 0
            time.sleep(0.5)  # 每0.5秒執行一次
    ...
    def download_obstacle_imgs(self):
        ...
        urls = [front_camera_url, back_camera_url,
                left_camera_url, right_camera_url]
        files = ["front_camera.jpg", "back_camera.jpg",
                 "left_camera.jpg", "right_camera.jpg"]
                ...
                if response.status_code == 200:
                    ...
                    with open(local_filname, "wb") as f:
                        for chunck in response.iter_content(chunk_size=8192):
                            f.write(chunck)
                    self.logger.debug(
                        f"File downloaded succesfully as {local_filname}, url:{url}")
                else:
                    self.logger.error(
                        f"Failed to download file, url:{url}. HTTP Status Code:{response.status_code}")
    ...
    def ftp_upload_obstacle_imgs(self, task_id):
        ...
        with MyFTP() as ftp:
            # login
            try:
                ftp.connect(self.camera.ftp.ftp_ip,
                            port=self.camera.ftp.ftp_port)
                ftp.login(self.camera.ftp.ftp_account,
                          self.camera.ftp.ftp_password)
            except:
                ...
                self.logger.debug(
                    f"ret_base:{ret_base}, base_folder:{base_folder}")
                if ret_base:
                        ...
                        if ret_task:
                            ...
                            if ftp.change_dir(task_id):
                                ...
                                local_files = os.listdir(
                                    "obstacle_imgs" + os.sep + task_id)
                                for local_file in local_files:
                                    if not local_file in remote_files:
                                        tasks.append(
                                            "obstacle_imgs" + os.sep + task_id + os.sep + local_file)
                                ...
                                if len(tasks) > 0:
                                    ...
                                    for task in tasks:
                                        if ftp.upload_file(task, os.path.basename(task)):
                                            success_cnt += 1
                                            self.logger.debug(
                                                f"upload ok, success_cnt:{success_cnt}.")
                                        else:
                                            self.logger.error(
                                                f"upload failed, success_cnt:{success_cnt}, stopped!")
                                            break
                                    if success_cnt == len(tasks):
                                        success = True
                                else:
                                    self.logger.debug(
                                        f"no images needed to be upload!")
                                    success = True
                            else:
                                self.logger.error(
                                    "change to task id folder failed!")
                        else:
                            self.logger.error(
                                "retrieve task id folder failed!")
                    else:
                        self.logger.error(
                            "change to obstancle_imgs folder failed!")
                else:
    ...
    def ftp_remove_obstacle_imgs(self, task_id):
        ...
        self.logger.debug(
            f"ftp remove obstacle imgs complete, satus:{status}, message:{message}")
...
def run_panorama_task(camera):
    ...
    write_file(
        filename, f'task_time:{camera.panorama_task.start_time.strftime("%Y-%m-%d %H:%M:%S")}\n')
...
"""
        ...
        data = ("panorama", camera.pos_folder_x, camera.pos_folder_y, camera.pos_folder_z,
                camera.pos_folder_theta, camera.pos_folder_tag_id, "save_imgs" +
                os.sep+camera.pos_folder+os.sep+camera.task_folder,
...
def run_target_task(camera):
    ...
    write_file(filename, f"ftp_url:{task_folder}\n")
    write_file(
        filename, f'task_time:{camera.target_task.start_time.strftime("%Y-%m-%d %H:%M:%S")}\n')
    ...
    lines = read_lines("save_imgs" + os.sep + camera.pos_folder +
                       os.sep + camera.task_folder + os.sep + "info.txt")
    stitch_state = "none"
    for line in lines:
...
def run_designated_task(camera):
    ...
    write_file(filename, f"ftp_url:{task_folder}\n")
    write_file(
        filename, f'task_time:{camera.designated_task.start_time.strftime("%Y-%m-%d %H:%M:%S")}\n')
...
def run_ir_task(camera):
    ...
    write_file(filename, f"ftp_url:{task_folder}\n")
    write_file(
        filename, f'task_time:{camera.ir_task.start_time.strftime("%Y-%m-%d %H:%M:%S")}\n')

    camera.ir_task.is_running = True

    try:
        cv2.imwrite(task_folder+os.sep+"ir.jpg", camera.ir_cam.get_img())
        cv2.imwrite(task_folder+os.sep+"ir-colormap.jpg",
                    camera.ir_cam.get_colormap_img(mark_max_temp=True))
    except Exception as e:
        print(e.args)
    ...
    print("uploading imgs...")
    print(f"pos_id:{camera.pos_folder}, task_folder:{camera.task_folder}")
    camera.main_logger.debug(
        f"uploading imgs..., pos_id:{camera.pos_folder}, task_folder:{camera.task_folder}")
    ret = ftp_upload_imgs(camera, camera.pos_folder, camera.task_folder)
    if ret:
...
def run_video_task(camera):
    ...
    write_file(filename, f"ftp_url:{task_folder}\n")
    write_file(
        filename, f'task_time:{camera.video_task.start_time.strftime("%Y-%m-%d %H:%M:%S")}\n')
    ...
    while camera.video_task.qsize() > 0:
        ...
        camera.main_logger.debug(
            f"pan:{pan}, tilt:{tilt}, zoom:{zoom}, video_time:{video_time}")
        ptz_angle = [pan, tilt, zoom]
        camera.main_logger.debug(
            f"ptz_angle:{ptz_angle}, video_time:{video_time}")
        ret = move_to_abs(camera, ptz_angle)  # PTZ控制
        if not ret:
            # print(f"ptz:{ptz_angle} retrived image failed!")
            camera.main_logger.error(
                f"ptz:{ptz_angle} retrieved image failed!")
        else:
            # make video
            camera.main_logger.debug(f"make video for {video_time}s")
            camera.make_video(video_time, task_folder +
                              os.sep + f"output_{video_index}.mp4")
            try:
        ...
        camera.main_logger.error(
            f"video task left cnt:{camera.video_task.qsize()}")
    ...
    # print(f"pos_id:{camera.pos_folder}, task_folder:{camera.task_folder}")
    camera.main_logger.debug(
        f"uploading videos..., pos_id:{camera.pos_folder}, task_folder:{camera.task_folder}")
    ret = ftp_upload_imgs(camera, camera.pos_folder, camera.task_folder)
    if ret:
        camera.main_logger.debug(
            f"uploading videos successfully, and write to mysql.")
        # write to mysql
        query = """
insert into `task_history` (`task_type`, `amr_pos_X`, `amr_pos_y`, `amr_pos_z`,
`amr_pos_theta`, `amr_tag_id`, `ftp_url`, `task_time`, `stitch_state`, `requestor`) values
...
"""
        ...
        if camera.mysql_conn.UpdateRowsByTuple(query, data):
            # 上傳成功刪圖像
            camera.main_logger.debug(
                "insert mysql successfully, ftp remove imgs")
            ftp_remove_imgs(camera, camera.pos_folder, camera.task_folder)
    else:
...
def ftp_upload_imgs(camera, pos_id, task_id):
    ...
    camera.main_logger.info(
        f"starting upload images, pos_id={pos_id}, task={task_id}")
    success = False
    with MyFTP() as ftp:
        # login
        try:
                ...
                if ftp.change_dir("save_imgs"):
                    ...
                    if ret_pos:
                        ...
                        if ftp.change_dir(pos_id):
                            ...
                            if ret_task:
                                ...
                                if ftp.change_dir(task_id):
                                    ...
                                    if ret:
                                        ...
                                        tasks = []
                                        local_files = os.listdir(
                                            "save_imgs" + os.sep + pos_id + os.sep + task_id)
                                        print(f"local_files:{local_files}")
                                        for local_file in local_files:
                                            if not local_file in remote_files:
                                                tasks.append(
                                                    "save_imgs" + os.sep + pos_id + os.sep + task_id + os.sep + local_file)
                                        # print(f"tasks:{tasks}")
                                        camera.main_logger.info(
                            ...
                            camera.main_logger.error(
                                "change to pos id folder failed!")
                    else:
                        camera.main_logger.error(
                            "retrieve pos id folder failed!")
                else:
                    camera.main_logger.error(
                        "change to save_imgs folder failed!")
            else:
...
def ftp_remove_imgs(camera, pos_id, task_id):
    """FTP上傳完成刪除照片"""
    camera.main_logger.debug(
        f"ftp remove imgs, pos_id:{pos_id}, task_id:{task_id}...")
    status, message = False, ""
    ...
    camera.main_logger.debug(
        f"ftp remove imgs complete, satus:{status}, message:{message}")
    return status, message
...
def ftp_upload_previous_tasks(camera):
    ...
    for pos in os.listdir("save_imgs"):
        for task in os.listdir("save_imgs" + os.sep + pos):
            ...
            if ret1:
                camera.main_logger.debug(
                    f"uploading successfully, pos:{pos}, task:{task}.")
                # read info.txt
                info_txt = "save_imgs" + os.sep + pos + os.sep + task + os.sep + "info.txt"
                camera.main_logger.debug(f"read info.txt, path{info_txt}.")
                try:
                    lines = read_lines(
                        "save_imgs" + os.sep + pos + os.sep + task + os.sep + "info.txt")
                except Exception as e:
                    camera.main_logger.error(
                        f"read task folder failed, pos:{pos}, task:{task}, error:{e}.")
                else:
                    ...
                    camera.main_logger.debug(
                        f"task_type:{task_type}, amr_pos_x:{amr_pos_x}, amr_pos_y:{amr_pos_y}, amr_pos_z:{amr_pos_z}, amr_pos_theta:{amr_pos_theta}, amr_tag_id:{amr_tag_id}, ftp_url:{ftp_url}, stitch_state:{stitch_state}, requestor:{requestor}, task_time:{task_time}")
    ...
    """
                    camera.main_logger.debug(f"query:{query}")
                    data = (task_type, amr_pos_x, amr_pos_y, amr_pos_z, amr_pos_theta,
                            amr_tag_id, ftp_url, task_time, stitch_state, requestor)
...
# camera offset
camera.camera_offset = int(
    config_obj.get_config_data("camera", "camera_offset"))
...
@app.route("/web/joy", methods=["GET", "POST"])
def joy():
    """amr control map"""
    # 渲染模板
    return render_template("joy2.html", camera=camera)

@app.route("/web/joystick_web", methods=["GET", "POST"])
def joystick_web():
    """joystick web"""
    # 渲染模板
    return render_template("joystick_web.html", camera=camera)
...
def get_camera_status():
    ...
    ptz = {"crr_pan": camera.crr_pan,
           "crr_tilt": camera.crr_tilt, "crr_zoom": camera.crr_zoom}
    task = {"ir_task": camera.ir_task.is_running, "target_task": camera.target_task.is_running, "designated_task":
            camera.designated_task.is_running, "inital_task": camera.initial_task.is_running, "video_task": camera.video_task.is_running}
    ...
    if request.args:
        ...
        try:
            ...
            try:
                ...
                ret = camera.onvif.abs_move(
                    ptz_value[0], ptz_value[1], ptz_value[2])
            except Exception as e:
...
def move_amr():
    ...
    if request.args:
        ...
        try:
            ...
            if cmd == "joystick":
                ...
                ret = camera.amr.move_amr(cmd, angle, strength)
            else:
    ...
    return jsonify(data)
@app.route("/amr/change_mode_joy/",  methods=['POST'])
def change_mode_joy():
    try:
          
        camera.amr.change_mode_joy()
        status = True
    except Exception as e:
        status = False
    data = {"status": status}
    return jsonify(data)
@app.route("/amr/move_amr_joy/",  methods=['POST'])
def move_amr_joy():
    """move amr"""
    global ControlKey 
    cmd=""
    ...
    angleDegrees = request.json.get('angleDegrees')
   

    status=False
    try: 
        key=request.json.get('ControlKey')
        #print(1)
        if key==ControlKey:
            status=camera.amr.move_amr(cmd, angleDegrees, strength)
    except Exception as e:
        camera.main_logger.error(f"move amr failed, error:{e}")
        status = False
    #ManualSemiControlDisableStatus
    camera.main_logger.debug(f"move amr status:{status}")
    data = {"status": status, "cmd": cmd,"strength":strength,"angleDegrees":angleDegrees,"ControlKey":ControlKey,"AutoManualStatus":camera.amr.AutoManualStatus,"ManualSemiControlDisableStatus":camera.amr.ManualSemiControlDisableStatus,"bWeb_bBeckhoff_IPC_HartBitStatus":camera.amr.bWeb_bBeckhoff_IPC_HartBitStatus}
    return jsonify(data)

# @app.route("/amr/change_to_manual_mode/", methods=["GET", "POST"])
# def change_to_manual_mode():
#     """change_to_manual_mode"""
#     camera.main_logger.debug("change to manual mode!")
#     status, cmd = False, ""
#     if request.args:
#         camera.main_logger.debug(f"args:{request.args}")
#         try:
#             cmd = request.args.get("cmd")
#             ret = camera.amr.change_to_manual_mode(cmd)
#         except Exception as e:
#             camera.main_logger.error(
#                 f"change amr manual mode failed, error:{e}")
#         else:
#             status = ret
#             # status = True
#     # camera.main_logger.debug(f"move amr status:{status}")
#     data = {"status": status, "cmd": cmd}
#     return jsonify(data)
@app.route('/web/setkey', methods=['POST'])
def setkey():
    global ControlKey 
    ControlKey = request.json.get('ControlKey')
    print('ControlKey')
    print(ControlKey)
    response_data = {
        'status': True,
...
def download_and_upload_obstacle_imgs():
    ...
    try:
        ...
        camera.main_logger.error(
            f"download and upload obstacle imgs failed, error:{e}")
        message = e
    else:
        status = True
    camera.main_logger.debug(f"move amr status:{status}")
...
def update_click_position():
    ...
    ret = move_to_abs(
        camera, [pan+target_pan, tilt+target_tilt, zoom+target_zoom])
...
"""
    ...
    camera.main_logger.debug(
        f"amr_tag_id:{camera.amr.amr_tag_id}, tasks:{tasks}")
    if not tasks:
    ...
    for task in tasks:
        ...
        theta = task[7]
        camera.main_logger.info(
            f"theta:{theta}, camera_offset:{camera.camera_offset}, pan:{pan}")
        ...
        target_pos_converted = convert_angle(target_pos)
        camera.main_logger.info(
            f"theta_converted:{theta_converted}, camera_pos_converted:{camera_pos_converted}, target_converted:{target_pos_converted}")
        ...
        camera.main_logger.info(
            f"input_theta_converted:{input_theta_converted}, input_camera_pos_converted:{input_camera_pos_converted}, input_pan:{input_pan}")

        camera.main_logger.info(
            f"input pan:{input_pan}, input tilt:{task[2]}, input zoom:{task[3]}")
        if input_pan > 170:
            ...
            if reverse_input_pan >= -170 and reverse_input_pan <= 170:
                camera.designated_task.put(
                    (reverse_input_pan, task[2], task[3]))
                camera.main_logger.debug(f"add command ok!")
            else:
                camera.main_logger.error(
                    f"input pan:{input_pan}, is not a valid angle, pass")
        elif input_pan < -170:
            # calculate reverse directon
            reverse_input_pan = input_pan + 360  # reverse direction
            camera.main_logger.info(f"reverse input pan:{reverse_input_pan}")
...
"""
    ...
    for task in tasks:
        ...
        video_time = task[9]
        camera.main_logger.info(
            f"theta:{theta}, camera_offset:{camera.camera_offset}, pan:{pan}, video_time:{video_time}")
        ...
        if input_pan > 170:
            ...
            camera.main_logger.info(f"reverse input pan:{reverse_input_pan}")
            if reverse_input_pan >= -170 and reverse_input_pan <= 170:
                camera.video_task.put(
                    (reverse_input_pan, task[2], task[3], video_time))
...
# for 20240722 demo 2nd source
@app.route("/camera/camera_video_feed1")
def camera_video_feed1():
    return Response(gen_camera_video(camera),
                    mimetype='multipart/x-mixed-replace; boundary=frame')
...
# for 20240722 demo 2nd source
@app.route("/front_camera/front_camera_video_feed1")
def front_camera_video_feed1():
    return Response(gen_front_camera_video(camera),
                    mimetype='multipart/x-mixed-replace; boundary=frame')
...
# for 20240722 demo 2nd source
@app.route("/ir/ir_camera_video_feed1")
def ir_camera_video_feed1():
    return Response(gen_ir_camera_video(camera),
                    mimetype='multipart/x-mixed-replace; boundary=frame')
